...
export async function getCategories(poblacion?: string): Promise<Category[]> {
  const qs = poblacion ? `?poblacion=${encodeURIComponent(poblacion)}` : ''
...
export async function getEvento(
  ...
  const catNombre = (c: {
    ...
    (lang === 'ca'
      ? (c.nombre_cat ?? c.nombre_es)
      : (c.nombre_es ?? c.nombre_cat)) ??
    c.slug ??
    ''
  ...
  return {
    ...
    descripcion: pickLang(
      raw.descripcion_larga_es,
      raw.descripcion_larga_cat,
      lang
    ),
