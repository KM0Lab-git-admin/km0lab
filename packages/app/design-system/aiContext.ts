...
export function generateAIContext(): string {
  ...
  typography.forEach((t) => {
    lines.push(
      `| \`${t.className}\` | ${t.family} | ${t.weight} | ${t.usage} |`
    )
  })
  lines.push('')
  ...
  typeScale.forEach((s) => {
    lines.push(
      `| \`${s.className}\` | ${s.px} | ${s.lineHeight} | ${s.usage} |`
    )
  })
  lines.push('')
  ...
  spacingScale.forEach((s) => {
    lines.push(
      `| \`gap-${s.token}\` / \`p-${s.token}\` | ${s.px} | ${s.usage ?? ''} |`
    )
  })
  lines.push('')
  ...
  animations.forEach((a) => {
    lines.push(
      `| ${a.name} | \`${a.className}\` | ${a.duration} | ${a.usage} |`
    )
  })
  lines.push('')
  ...
  lines.push(
    `- Tamaños semánticos: sm=${iconography.sizes.sm}, md=${iconography.sizes.md}, lg=${iconography.sizes.lg}, xl=${iconography.sizes.xl}`
  )
  ...
  lines.push('}`}>')
  lines.push(
    `  {hasError ? <MapPinOff className="text-destructive" size={22} /> : <MapPin className="text-km0-teal-500" size={22} />}`
  )
  lines.push(
  ...
  lines.push(
    `  <div className="landscape:hidden flex flex-col gap-6 vertical-mobile:gap-7">…</div>`
  )
  lines.push(
  ...
  ;(Object.keys(CATEGORY_LABELS) as ComponentCategory[]).forEach((cat) => {
    ...
    items.forEach((c) => {
      ...
      if (c.responsive.length) {
        ...
        c.responsive.forEach((r) =>
          lines.push(`- \`${r.breakpoint}\`: ${r.behavior}`)
        )
      }
      if (c.notes?.length) {
        lines.push('')
