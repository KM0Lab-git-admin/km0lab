...
const REFERENCE_URL =
  process.env.REFERENCE_URL ?? 'https://char-con-todos.lovable.app/'
...
const READY_SELECTOR_TEXT = 'Escoge tu idioma'
...
const outputRoot = path.resolve(__dirname, '../artifacts/language-screen')

const IGNORED_REGIONS_BY_VIEWPORT = {
  // Overlay externo de la página de referencia (no pertenece al producto).
  'horizontal-mobile-667x375': [{ x: 505, y: 338, width: 162, height: 37 }],
}
...
async function preparePage(page, url) {
  ...
  await page
    .getByText(READY_SELECTOR_TEXT, { exact: false })
    .first()
...
function maskIgnoredRegions(image, viewportName) {
  const regions = IGNORED_REGIONS_BY_VIEWPORT[viewportName] ?? []
...
async function main() {
  ...
  const localUrl = process.env.LOCAL_URL ?? server.url
  ...
  try {
    for (const viewport of viewports) {
      ...
      const referencePng = PNG.sync.read(await fs.readFile(referencePath))
      const localPng = PNG.sync.read(await fs.readFile(localPath))
      maskIgnoredRegions(referencePng, viewport.name)
      maskIgnoredRegions(localPng, viewport.name)
      await fs.writeFile(referencePath, PNG.sync.write(referencePng))
      await fs.writeFile(localPath, PNG.sync.write(localPng))
